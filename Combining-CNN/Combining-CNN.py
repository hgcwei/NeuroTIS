...
with tf.device('/cpu:0'):
    ...
    x1, x2, x3 = tf.split(x, [1, 2, 4], 2)
    x11, x12 = tf.split(x1, [17, 301-17], 1)
    ...
    pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2, 1], strides=3)
    ...
    conv2 = tf.layers.conv2d(
        ...
        activation=tf.nn.relu,
        kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
    pool2 = tf.layers.max_pooling2d(inputs=conv2, pool_size=[2, 1], strides=3)
    drop2 = tf.layers.dropout(pool2, 0.3)
    # # 第三个卷积层(99->49)

    re1 = tf.reshape(drop2, [-1, 71*100])
    re2 = tf.reshape(x11,[-1,17])
    re3 = tf.reshape(x2,[-1,301*2])

    co1 = tf.concat([re1, re2, re3], axis=1)
    # 全连接层
    dense1 = tf.layers.dense(inputs=co1,
                             units=20,
                             activation=tf.nn.relu,
                             kernel_initializer=tf.truncated_normal_initializer(stddev=0.01)
                             )
